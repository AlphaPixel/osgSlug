...
// Per-shape data that was previously replicated across every vertex.
// Indexed by (a_position.w - 1); the same 1-based layer index already carried
// in the position attribute, converted to 0-based here.
struct ShapeData {
	...
	vec4 effectData;    // x = effectId, yz = originX/Y, w = worldWidth
...
void main() {
	ShapeData sd = shapes[int(a_position.w + 0.5) - 1];

	vec3 pos = a_position.xyz;
	float t = osg_SimulationTime;
	int eid = int(sd.effectData.x + 0.5);
	int mode = eid / 100;
	int index = eid % 100;
	...
	if(mode == 7) {
		...
		float W = sd.effectData.w; // world width from SSBO
	...
	if(eid == 8) {
		...
		mat2 R = mat2(c, s, -s, c);
		vec2 origin = pos.xy - a_emCoord.xy + sd.effectData.yz; // origin from SSBO
	...
	v_effectId = eid;
	v_gradientId = int(sd.gradientMeta.x + 0.5);
	v_gradientMeta = sd.gradientMeta;
	v_gradientXform = sd.gradientXform;
