...
//
// SHADER_VERT / SHADER_VERT_GL3 / SHADER_VERT_DECAL contain #pragma osgSlug lib_vertex
// which makeVertShader resolves at use time (injects osgSlug_VertexData + helpers).
//
// SHADER_FRAG is stored pre-resolved (resolveLibs run at static init time) so callers
// such as InkDrawable.cpp can use it directly without an additional resolve step.
// ------------------------------------------------------------------------------------------------

// Main SSBO-based vertex shader. SHADER_TYPES (AtlasShapeData + LayerData) is injected
// by makeVertShader after the #version line; #pragma osgSlug lib_vertex is resolved there too.
...
// Tangent-plane sphere decal vertex shader. SHADER_ATLAS_TYPES is injected
// by makeVertShader (binding 0 only); #pragma osgSlug lib_vertex resolved there.
...
// Pre-resolved so SHADER_TYPES injection by makeVertShader picks up osgSlug_VertexData from the
// already-substituted lib content.
...
// The FRAGMENT shader injection point; controls the per-covered-fragment color.
// Pre-resolved so InkDrawable.cpp can use it directly without an additional resolve step.
...
// Default for osgSlug_FragmentExt: contributes nothing, so drawables render identically
// to before the hook existed unless they opt in. Pre-resolved; usable directly.
